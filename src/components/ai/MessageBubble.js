...
import { Copy, Zap, CheckCircle2, AlertCircle, Loader2, ChevronRight, Clock, Package } from 'lucide-react';
...
export default function MessageBubble({ message }) {
  ...
  const processContent = (content) => {
    ...
    const artifactIdPattern = /\[ARTIFACT_ID:\s*([a-zA-Z0-9-]+)\]/g;
    const parts = [];
    let lastIndex = 0;
    let match;
    
    while ((match = artifactIdPattern.exec(content)) !== null) {
      if (match.index > lastIndex) {
        parts.push({ type: 'text', content: content.slice(lastIndex, match.index) });
      }
      
      parts.push({ type: 'artifact', id: match[1] });
  ...
  const contentParts = isUser ? null : processContent(message.content);
          ...
          <div className={cn(
            ...
            {isUser ? (
              ...
              <div className="text-sm prose prose-sm prose-slate max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
                  ...
                  if (part.type === 'text') {
                    ...
                    return (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        onClick={() => navigate(`${createPageUrl("AdminArtifacts")}?artifact=${part.id}`)}
                        className="inline-flex items-center gap-2 bg-purple-50 border-purple-300 hover:bg-purple-100 text-purple-700 my-1 mr-2"
                      >
                ...
                <ReactMarkdown 
                    className="text-sm prose prose-sm prose-slate max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_strong]:font-bold [&_h1]:text-lg [&_h1]:font-bold [&_h2]:text-base [&_h2]:font-bold [&_h3]:text-sm [&_h3]:font-bold"
                    components={{
                  code: ({ inline, className, children, ...props }) => {
                    const match = /language-(\w+)/.exec(className || '');
                    return !inline && match ? (
